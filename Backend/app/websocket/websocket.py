...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket):
        ...
        logger.info(
            f"Client connected: {client_id}. Total connections: {len(self.active_connections)}"
        )
    ...
    def disconnect(self, websocket: WebSocket):
        ...
        if client_id in self.active_connections:
            ...
            logger.info(
                f"Client disconnected: {client_id}. Remaining connections: {len(self.active_connections)}"
            )
    ...
    async def send_error(self, websocket: WebSocket, error_message: str):
        ...
        await self.send_personal_message(
            {"type": "error", "payload": {"message": error_message}}, websocket
        )

    async def handle_message(
        self, message: WebSocketCommand, websocket: WebSocket
        ...
        if action in self.message_handlers:
            try:
                ...
                return await self.message_handlers[action](payload, websocket)
            except Exception as e:
